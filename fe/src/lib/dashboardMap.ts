...
export type MapCenter = { lat: string; lng: string };
...
function wireZoomButtons() {
	...
	document.getElementById('map-recenter')?.addEventListener('click', () => {
		if (map) map.setView(lastCenter, Math.max(map.getZoom(), 12), { animate: true });
	});
}
...
export function plotStoresOnMap(stores: Store[], mc: MapCenter | null | undefined) {
	ensureMap();
	...
	stores.forEach((s, i) => {
		...
		L.circleMarker([lat, lng], {
			...
			.bindPopup(`<strong>${name}</strong><br/><span style="opacity:.85">${phone}</span>`)
			.addTo(resultsLayer);
	});
	scheduleInvalidate();
}
